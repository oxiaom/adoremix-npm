(function () {  
    ...
    var appendToBuffer = function (mp3Buf) {  
	    console.log("appendToBuffer=>" + mp3Buf.length)
        dataBuffer.push(new Uint8Array(mp3Buf));  
    };   
  
    var init = function (prefConfig) {  
       //console.log(prefConfig)
        config = prefConfig || {};  
        lame = new lamejs();
        mp3Encoder = new lame.Mp3Encoder(1, config.sampleRate || 44100, config.bitRate || 320);  
        clearBuffer();  
        self.postMessage({  
            cmd: 'init'  
        });  
